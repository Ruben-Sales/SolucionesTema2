...
browseURL(url='https://archive.ics.uci.edu/ml/datasets/Wine') # Enlace de la página con información del conjunto de datos
...
fichero<-paste0('./data/UCIwineQuality','.txt') # Nombre que le queremos poner al fichero.
file<-'https://archive.ics.uci.edu/ml/machine-learning-databases/wine/wine.data' # Enlace para descargar el conjunto de datos.
download.file(url=file, destfile = fichero) # Descarga el conjunto de datos.
file<-'https://archive.ics.uci.edu/ml/machine-learning-databases/wine/wine.names' # Enlace del CodeBook del conjunto de datos
download.file(url=file, destfile = './data/wine.names') # Ruta local donde se guardará el CodeBook.
list.files('./data') # Mostrar los archivos en el directorio data.
...
```{r}
# b)
# En comparación al CodeBook perfecto, falta una breve definición de cada variable. Además, no se habla nada sobre valores perdidos. Tampoco se da ninguna indicación del uso del conjunto de datos. Además, no se indican los valores que puede tomar cada variable. El CodeBook carece de información estadística y de unidades de medida. Por último, tampoco se habla de los outliers.
...
```{r}
wine.names<-readLines(con='data/wine.names')
str(wine.names)
...
3.  En este ejercicio veremos el procedimiento para conectarnos a una base de datos remota. Esta base de datos se utiliza en el curso DataCamp **Importing Data in R (part2)**. Es necesario tener instalada la librería **RMySQL**. El siguiente código muestra como establecer la conexión con una base de datos llamada **tweater** que se encuentra en un host remoto **courses.csrrinzqubik.us-east-1.rds.amazonaws.com** y disponemos el puerto, usuario y contraseña para poder acceder.
